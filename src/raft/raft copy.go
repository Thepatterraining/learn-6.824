...
// 	log.Printf("raft %d, 复制日志结果：%v", rf.me, rf.log)
// 	// 5. If leaderCommit > commitIndex, set commitIndex = min(leaderCommit, index of last new entry)
...
// 	index := len(rf.log)
// 	log.Printf("raft %d 接收到日志命令 %v log %v", me, command, rf.log)
// 	if !isLeader {
// 		rf.mu.Unlock()
...
// 			log.Printf("raft %d 更新后 server %d match index %d next index %d", rf.me, peer, rf.matchIndex[peer], rf.nextIndex[peer])
...
// 					// 多数同意，就提交
// 					log.Printf("raft %d 日志复制返回成功 多数同意，提交日志给KV服务器 for raft %d N %d term %d reply:%v", rf.me, peer, N, rf.currentTerm, reply)
...
// 						log.Printf("raft %d 成为Leader", rf.me)
// 						rf.status = Leader
// 						// 初始化nextIndex
// 						for i := range rf.peers {
